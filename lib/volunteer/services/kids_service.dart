...
class VolunteerKidsService {
  Future<KidRecentDto> fetchRandomRecentKid() async {
    ...
    try {
      final response = await RequestClient.client.get(
        Uri.parse('$baseUrl/volunteer/kid/random/recent'),
        );


      if (response.statusCode == 200) {
        print("kids recent gotten, ${response.statusCode}");
        var data = json.decode(response.body)['data'];
        return KidRecentDto.fromJson(data);
      } else {
